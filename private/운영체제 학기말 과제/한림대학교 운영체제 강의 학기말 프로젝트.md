...
프로그램의 동작 - 관리자 명령어, 사용자 명령어 - 를 담당하는 모든 클래스는 Command 클래스를 상속한다.
...
그에 더해, NotConnected 예외를 발생시킬 수 있는 명령어들은 ClientCommandNeedName을 상속한다.
...
2. 서버(자식 프로세스)들이 접근할 수 있는 공유 메모리와 이를 보호할 pthread_rwlock을 초기호
