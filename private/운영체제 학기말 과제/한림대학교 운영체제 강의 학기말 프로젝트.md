...
1. a 서버의 사용자가 b 서버의 사용자를 대상으로 isc를 요청한다.
2. a 서버는 iscSend 객체를 만든다. iscSend는 b 서버의 정보를 공유메모리에서 조회하고 b와 연결되는 소켓을 만든다. 그리고 그 소켓을 통해 iscRecieve 요청을 b 서버에 보낸다.
3. iscRecieve 요청을 받은 b 서버는 수신한 문자열을 파싱하여 isc를 받을 사용자의 file descritor와 메세지 본문을 알아낸다. 그리고 그 사용자에게 메세지를 송신한다.
...
Channel 클래스는 채널의 관리자, 채널의 멤버 정보를 관리하는 객체이다. channelMsg, refer 명령어는 Channel 클래스에 저장되어 있는 멤버 정보를 사용하여 그 기능을 구현한다.
join, leave, kick 등 채널의 정보를 변경시키는 명령어들은 Channel 클래스에서 제공하는 여러 멤버 함수를 통해 그 기능을 구현한다.
